...
Al la serie de eventos $e_{i}$  que se colocan dentro de un procesos $p_{i}$ se le llama \gls{historia del proceso} \sidecite{Coulouris2011} , ordenado  por la relación ${i}:$ \\  $history(p_{i})  = h_{i}  = <e_{i}^{0},  <e_{i}^{1},  <e_{i}^{2}, ....>  $ .
...
\subsection{Relojes F\'isicos }	
	...
	\subsubsection{UTC}
 	\index{UTC}
		...
		El Algoritmo de Cristian \sidecite{Cristian1989}  propone sincronizar un conjunto de relojes de 	máquinas a partir de una que esté sincronizada externamente (tiempo u hora real), a través de la red de comunicaciones de datos entre computadoras. 
...
\paragraph{Problemas con el Algoritmo de Cristian} 
		 ...
		 \end{figure}
			...
			%--------------------------------------------------------------------
		 Algoritmo de Bekerley \sidecite{Gusella1985} es un algoritmo centralizado o de sincronizaci\'on interna, en el que  se elige un servidor  entre todos los equipos que se encuentran conectados en el entorno. Este servidor toma el rol de maestro o  servidor de tiempo y los servidores conectados son esclavos.  
	  ...
	  \begin{itemize} 
		...
		En \sidecite{Mills1995} Mills define un protocolo llamado NTP (NTP, Network Time Protocol) que es una una arquitectura y protocolo para distribuir «relojes»  
	...
	\end{itemize}
		...
		Para el intercambio de mensajes   entre los servidores se usa datagramas UDP.
			...
			Lamport,  \sidecite{Raptis2020} \sidecite{Czaja2018} \sidecite{Lamport1978},  señala que la sincronización de relojes no tiene que ser absoluta. Si dos procesos no interactúan no es necesario que sus relojes estén sincronizados. 
	...
	\paragraph{Problemas con el algoritmo de Lamport}
			...
			%--------------------------------------------------------------------
		 En distintos documentos, \sidecite{Mattern1989} y \sidecite{Fidge1991} proponen  una solución para superar las dificultades que presenta el Algoritmo de relojes l\'ogicos de Lamport. 
...
\index{consenso distribuido}
   ...
   La exclusión mutua garantiza que los procesos concurrentes   puedan compartir recursos. A continuaci\'on  se presentanlos las soluciones  relacionados con el problema de la exclusión mutua \sidecite{Wu1998} \sidecite{Czaja2018}.
...
Estos algoritmos logran la exclusión mutua pasando un mensaje especial entre los procesos llamado token \sidecite{Steen2017}. Solo hay un token disponible y solo aquel proceso que lo tenga podrá entrar en la región crítica.
...
Una solución simple para la exclusión mutua distribuida utiliza un coordinador \sidecite{Steen2017} \sidecite{Coulouris2011} \sidecite{Wu1998}. Para cada proceso que solicita el acceso a la
...
%----------------------------------------------------------------------
 El algoritmo de Anillos de Procesos \sidecite{Coulouris2011} \sidecite{Steen2017}, figura \ref{fig:alg-anillo} opera de la siguiente manera: 
	\begin{itemize} 
...
\index{ algoritmo de Lamport de exclusi\'on mutua}
 ...
 Es un algoritmo basado en permisos propuesto por Lamport, \sidecite{Lamport1978} como una ilustración de su esquema de sincronización para sistemas distribuidos. Es un  mecanismo basado en relojes lógicos para el orden  total de las peticiones en el sistema.
...
%----------------------------------------------------------------------
 El algoritmo Ricart-Agrawala es un algoritmo 	de exclusión mutua en un sistema distribuido propuesto 	por \textit{Glenn Ricart} y  \textit{Ashok Agrawala} \sidecite{Ricart1981} . 
...
El Rendimiento:
	...
	El algoritmo de Suzuki-Kazami \sidecite{Suzuki1985} es una modificación del algoritmo Ricart-Agrawala, un algoritmo basado en permisos   que utiliza mensajes de SOLICITUD y RESPUESTA para garantizar la exclusión mutua.
...
%----------------------------------------------------------------------
 En el algoritmo de Anillo basado en Elecciones \sidecite{Chang1979} se sigue el siguiente proceso:
...
%---------------------------------------------------------------------
 Las premisas del algoritmos de \textit{Bully} \sidecite{GarciaMolina1982}
...
%----------------------------------------------------------------- 
El problema de los generales bizantinos  \sidecite{Lamport1982}  plantea que un grupo de generales sitia una ciudad y deben ponerse de acuerdo a través de un plan de ataque para atacar o retirarse.
