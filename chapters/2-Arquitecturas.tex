...
Bajo esta óptica se puede identificar útilmente tres generaciones de distribuido sistemas \sidecite{Coulouris2011}. \index{modelos f\'isicos}
...
\begin{description}
	...
	\marginnote{
		\begin{kaobox}[frametitle= Sistemas distribuidos tempranos ]
			Son sistemas individuales,  en gran medida homogéneos y  la apertura no era la característica relevante. Las metas de diseño se enfocaban en la calidad del servicio. \CO
		\end{kaobox} 	}
	...
	\marginnote[-3 cm]{
		\begin{kaobox}[frametitle= Sistemas distribuidos a escala de internet ]
	...
	\begin{itemize}
		\item Arquitectura cliente-servidor. Puede presentar varios clientes conectados a un servidor, como el de la figura \ref{fig:ClienteServercomp}, varios clientes y 	servidores conectados a internet y conversando entre ellos. \index{cliente-servidor}
		
		\begin{figure}[h]%
			\includegraphics[width=0.7\textwidth] {2/ClienteServercomp.png}
			\caption{Arquitectura Cliente Servidor}
			\label{fig:ClienteServercomp}
		\end{figure}
		...
		\begin{figure}[h]%
			\includegraphics[width=0.8\textwidth]{2/P2P.png}
	...
	Ejemplos son sistemas que usan bases de datos distribuidas en tiempo real, como en \sidecite{Sakaki2010} donde se relaciona  las publicaciones que hacen  las personas en Twitter (tweets) relacionadas con un terremoto, lo que permite detectar rápidamente la ocurrencia de un terremoto y proponen un algoritmo para monitorear tweets y detectar un evento objetivo.  
	
	Y en \sidecite{Basha2008} presentan un sistema basado en  redes de sensores ambientales predictivos donde se presenta una aplicación para predecir de inundaciones de ríos.	
\end{description}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Los modelo arquitectónicos clasifican su contenido en componentes de la arqiuitectura: entidades, paradígmas de comunicación, roles y responsabilidades, y correspondencia con la infraestructura física \sidecite{Coulouris2011}.
...
\begin{description}	
	...
	Por ejemplo, en la figura \ref{fig:Arq-procesos} se ilustra la organización de una base de datos una red de monitoreo y los procesos que se ejecutan en ella, en el lugar del operador (a), o en los sensores (b). \sidecite{Tanenbaum2007}
	
	\begin{figure}%
		\includegraphics[width=0.8\textwidth]{2/procesos.png}
	...
	La figura \ref{fig:Arq-nodos} muestra la organización de los nodos en la arquitectura de Chord \sidecite{Stoica2001}, un protocolo y algoritmo para sistemas P2P con \gls{tablas hash distribuidas}   y que proporcina un servicio de búsqueda descentralizado que almacena pares clave/valor para esas redes. 
	
 	
 		\marginnote{
 			\begin{kaobox}[frametitle= Chord]
	...
	\begin{figure}%
		\includegraphics[width=0.8\textwidth]{2/nodos}
	...
	\begin{figure}%
		\includegraphics[width=0.8\textwidth]{2/hilos.png}
	...
	Por ejemplo, en la figura \ref{fig:Arq-object}, se muestra la constituci\'on de la arquitectura de \gls{Corba}, detallando los objetos y sus funciones \sidecite{Corba2022}. 

	\begin{figure}%
		\includegraphics[width=0.8\textwidth]{2/Arq-object.png}
...
%%	Se parecen a los objetos porque ofrecen abstracciones orientadas a problemas para construir sistemas distribuidos y también son accedido a través de interfaces. La diferencia es que los \gls{componentes} especifican no solo sus interfaces, también las supuestos que hacen en términos de otros componentes/interfaces que deben estar presentes para que un componente cumpla su función. \index{componentes} 
	...
	\begin{figure}%
		\includegraphics[width=0.7\textwidth]{2/Arq-componente.png}
	...
	Los \gls{servicios web} están estrechamente relacionado con objetos y componentes,  adoptando un enfoque basado en la encapsulación de comportamiento y acceso a través de interfaces. Están integrados en la \textit{World Wide Web}, utilizando estándares web para representar y descubrir servicios \sidecite{Erl2007}. 
	
	En \sidecite{W3C2022} lo definen como un sistema software diseñado para soportar la interacción máquina-a-máquina, a través de una red, de forma interoperable. Cuenta con una interfaz descrita en un formato procesable por un equipo informático (específicamente en WSDL), a través de la que es posible interactuar con el mismo mediante el intercambio de mensajes SOAP, típicamente transmitidos usando serialización XML sobre HTTP conjuntamente con otros estándares web.
	
	\marginnote[-3cm]{
	\begin{kaobox}[frametitle= Servicios WEB ]
...
%%		\caption{El servicio Agente de Viaje combinado con otros servicios Web. Tomado de \CO} 		\label{fig:Arq-servicio} 	\end{figure}
...
Considere tres tipos de paradigma de comunicación: comunicación entre procesos, Invocación remota y comunicación indirecta \sidecite{Steen2017} \sidecite{Elshebani2009} \sidecite{Silcock1995}  .
...
%%	\marginnote[-3.2cm]{
	...
	\begin{description}		
		...
		\begin{figure}[h]%
			\includegraphics[width=0.8\textwidth]{2/arq-pub-sub.png}
		...
		\begin{figure}[h]%
			\includegraphics[width=0.7\textwidth]{2/arq-cola.png}
...
En esta clasificaci\'on se considera  cómo las entidades, objetos o servicios se asignan o forman parte de la infraestructura física distribuida subyacente. Se exploran tres aspectos \sidecite{Coulouris2011}, \sidecite{Limoncelli2014}:
...
\begin{description}
	...
	\begin{figure}[H]%
		\includegraphics[width=0.7\linewidth]{2/Arq-mult-server.png}
	...
	\begin{figure}[h]%
		\includegraphics[width=0.7\linewidth]{2/Arq-web-proxy.png}
	...
	\begin{figure}%
		\includegraphics[width=0.7\linewidth]{2/Arq-web-applet.png}
...
\begin{description}
	...
	\begin{figure}%
		\includegraphics[width=0.7\textwidth]{2/arq-capas.png} 
		\caption{Arquitectura de capas de dos niveles.}
		\label{fig:Arq-capas}
	\end{figure}
	
	\begin{figure}%
 	\includegraphics[width=0.7\textwidth]{2/arq-capas-2.png}
...
\end{figure}
	...
	\begin{figure}[h]%
		\includegraphics[width=0.7\textwidth]{2/arq-clienteFlaco.png}
...
%	\begin{description}
		...
		\item[\textit{Brokerage}]
			\marginnote[-1cm]{
			\begin{kaobox}[frametitle= Brokerage]
...
%			\index{patrones!Balanceador de carga con múltiples réplicas de \textit{backend}}
	...
	%		\begin{figure}[h]%
		%		\includegraphics{loadBalancer.png} 				\caption{Balanceador de Cargas. Tomado de \LI}  				\label{fig:LoadBalance}  		\end{figure}
...
%			\begin{figure}[h]  				\includegraphics[width=0.8\textwidth]{ServerBackend.png} 				\caption{Servidores Replicados. Tomado de \LI}  				\label{fig:ServerBackend}  			\end{figure}
...
%		\begin{figure}[h]								
%				\includegraphics[width=0.8\textwidth]{ServerTree.png}			\caption{\'Arbol de Servidores. Tomado de \LI} 				\label{fig:ServerTree} 			\end{figure}
...
En la figura \ref{fig:DNS} se visualiza la estructura del árbol que caracteriza al DNS. Cada nodo representa nodos y las hojas pueden ser nodos o recursos. Los nodos de ubicados a la izquierda se refieren a los dominios genéricos: com, edu, org, gov; los nodos hacia la derecha se muestra los servidores de nombres dedicados a paises, por ejemplo: ac, al, co, pe, ve, entre otros. 
		...
		\begin{figure}									
			\includegraphics[width=0.8\textwidth]{2/DNS.png}		
...
Los modelos de navegación que soporta el DNS son los siguientes \sidecite{Coulouris2011}:
...
\begin{itemize}
	...
	\begin{figure}									
		\includegraphics[width=0.9\textwidth]{2/DNS-inter.png}		
	...
	\begin{figure}									
		\includegraphics[width=0.9\textwidth]{2/DNS-inter-serv.png}		
	...
	\item Navegación Recursiva controlada por el servidor.
	  En la navegación recursiva controlada por el servidor, ver figura \ref{fig:DNS-recur}, el cliente  contacta a un solo servidor. Si este servidor no almacena el nombre, el servidor contacta a un compañero que almacena un prefijo  del nombre, que a su vez intenta resolverlo. Este procedimiento continúa recursivamente hasta que se resuelve el nombre.
		...
		\begin{figure}									
		\includegraphics[width=0.7\textwidth]{2/DNS-recur.png}		
...
\begin{figure}									
	\includegraphics[width=0.7\textwidth]{2/DNS-busq.png}		
